...
from utils.loss import CrossEntropy2d, BCEWithLogitsLoss2d, BCEWithLogitsLoss2dd
...
def compute_gradient_penalty(real_target, fake_target, D):
    alpha = torch.rand((real_target.size(0), 1, 1, 1)).cuda()
    interpolates = (alpha * real_target + ((1 - alpha) * fake_target)).requires_grad_()
    d_interpolates = D(interpolates)
    fake_label = torch.ones(d_interpolates.shape).cuda()
...
def main():
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            for i in range(10):
                ...
                try:
                    ...
                    pass;
                imagess = torch.from_numpy(cv2.Laplacian(imagess.cpu().detach().numpy(), -1)).cuda()
                labell = torch.from_numpy(cv2.Laplacian(label.cpu().detach().numpy(), -1)).cuda()
        ...
        print('iter = {0:8d}/{1:8d}, loss_seg = {2:.3f}, loss_adv_p = {3:.3f}, loss_D = {4:.3f}, loss_semi = {5:.3f}, loss_semi_adv = {6:.3f}, loss_laplacian = {7:.3f}'.format(i_iter, args.num_steps, loss_seg_value, loss_adv_pred_value, loss_D_value, loss_semi_value, loss_semi_adv_value, loss_laplacian))
